"""
plot_e2r_validation.py  (v2)
============================
E2R scramjet wall-pressure validation against Li et al. (2023), Fig. 9.
 
Fueled case: heat addition and mass addition happen simultaneously,
starting at x_fuel = 1.0 m and continuing through the diverging combustor.
"""
...
# ---------------------------------------------------------------------------
x3 = 1.2
x2 = x3 - L23          # 1.0  (isolator exit / const-area comb inlet)
x1 = x2 - L12          # 0.84
x4 = x3 + L34          # 1.84
x5 = x4 + L45          # 2.17
SECS = [x1, x2, x3, x4, x5]
 
# Fueled case: combustion starts at x_fuel = x2 = 1.0 m
X_FUEL = x2   # fuel injection / combustion start  (= 1.0 m)
 
print(f"Stations (m): x1={x1:.3f}  x2={x2:.3f}  x3={x3:.3f}  x4={x4:.3f}  x5={x5:.3f}")
print(f"Fuel/heat injection starts at x = {X_FUEL:.3f} m")
...
# Total combustion zone length: from X_FUEL to x4
# ---------------------------------------------------------------------------
L_COMB = x4 - X_FUEL   # total combustion length
 
def deta_dx(x, theta, dx=1e-5):
    """
    Mixing efficiency derivative — eta is defined over the full combustion
    zone [X_FUEL, x4], normalised by L_COMB.
    """
    def eta(xx):
        s = max(min((xx - X_FUEL) / L_COMB, 1.0), 1e-6)
        e0  = s
        e90 = max(min(1.01 + 0.2 * math.log(s), 1.0), 0.0)
        if theta == 0:   return e0
        if theta == 90:  return e90
        return theta / 90 * (e90 - e0) + e0
    return (eta(min(x + dx, x4)) - eta(max(x - dx, X_FUEL))) / (2 * dx)
...
def run_section(x0, x1_, W_in, W_out, H_in, H_out,
                ...
                Y, heat_fn=None, mass_rate=0.0, switches=None):
    L_ = x1_ - x0
    if switches is None:
    ...
    def geometry_fn(x):
        ...
        A  = W_ * H_
        dA = ((W_out - W_in) * H_ + W_ * (H_out - H_in)) / L_ if L_ > 0 else 0.
        D  = D_h(W_, H_)
        return A, dA, D
 
    def composition_fn(x, T, p): return Y
 
    def source_fn(x, T, p, m, _):
        return (heat_fn(x, T, p) if heat_fn else 0.), mass_rate
...
# ── Fueled θ=90: heat + mass addition together from X_FUEL = x2 ─────────────
#
#   Section A: X_FUEL → x3  (constant-area geometry, W2×H2)
#              mass_rate = MFUEL / L_COMB  (spread over full combustion zone)
#              heat_fn   = deta_dx(x, 90) * YF * Q_H2_HHV
#
#   Section B: x3 → x4     (diverging geometry)
#              same mass_rate and heat_fn
 
# Total fuel mass-flow rate spread uniformly across the combustion zone length
MASS_RATE_PER_M = MFUEL / L_COMB   # kg/s per metre
 
def h90(x, T, p):
    """Heat release rate per unit length [J/(kg·m)] using theta=90 mixing."""
    return YF * Q_H2_HHV * deta_dx(x, 90)
 
# Section A: constant-area combustor region (x2 → x3), W2×H2
rfA = run_section(
    X_FUEL, x3,
    W2, W2, H2, H2,          # constant area
    r12["Ma"][-1], r12["T"][-1], r12["p"][-1], MDOT1, Y_COMB,
    heat_fn=h90,
...
# Section B: diverging combustor (x3 → x4), W3→W4, H3→H4
rfB = run_section(
    x3, x4,
    W3, W4, H3, H4,           # diverging
    rfA["Ma"][-1], rfA["T"][-1], rfA["p"][-1], mdotA, Y_COMB,
    heat_fn=h90,
    mass_rate=MASS_RATE_PER_M,
)
...
# Nozzle: no combustion
rf45 = run_section(x4, x5, W4, W5, H4, H5,
                   rfB["Ma"][-1], rfB["T"][-1], rfB["p"][-1], mdotB, Y_COMB)
...
print(f"  Exp peak:      p/p0={EXP_F[:,1].max():.1f}  at x={EXP_F[EXP_F[:,1].argmax(),0]:.3f} m")
print(f"  Fuel mdot in={MFUEL:.4f}  mdot at exit of comb={mdotB:.4f}  (expected ~{MDOT1+MFUEL:.4f})")
