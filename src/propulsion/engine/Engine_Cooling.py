...
class RamjetCooling(Ramjet):
    ...
    def run_cycle(self, h: float, Ma0: float, m_air: float, phi: float = 0.5,
        ...
        self.m_air = m_air
        self.phi = phi
        self.m_fuel = max(phi * m_air / 34.3, 1e-9)
        results = super().altitude_mach(h_km=h, Ma0=Ma0)
    ...
    def cooling_analysis(self) -> dict:
        ...
        mfuel = float(getattr(self, "m_fuel", 1e-9))
...
if __name__ == "__main__":
    ...
    cool.run_cycle(h=20.0, Ma0=3.0, m_air=100.0, phi=0.5)
