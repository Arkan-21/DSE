...
def optimise_intake_fixed_L1(
    M_inf: float,
    L1: float,
    delta_cowl_deg: float,
    max_oblique_shocks: int,
    ycowl_range: tuple = (0.01, 2.0),
    theta1_range: tuple = (1.0, 20.0),
    ycowl_step: float = 0.005,
    theta1_step: float = 0.5,
    M_exit_min: float = 0.1,
    M_exit_max: float = 0.8,
    print_all: bool = False,
):
    """
    Optimises intake geometry for fixed L1.
    ...
    Objective:
        Maximise total pressure recovery.
    """

    if print_all:
        print(
            f"{'y_cowl':>10} {'theta1':>8} {'theta2':>8} "
            f"{'L2':>8} {'P0_rec':>12} "
            f"{'M_final':>10} {'Tot_Shocks':>10} {'Obliques':>9}"
        )
        print("-" * 95)

    best = None

    ycowl_values = np.arange(
        ycowl_range[0],
        ycowl_range[1] + ycowl_step,
        ycowl_step
    )
    ...
    for ycowl in ycowl_values:
        for theta1 in theta1_values:
            ...
            try:
                ...
                # -----------------------------
                # Exit Mach constraint
                # -----------------------------
                if not (M_exit_min <= r["M_exit"] <= M_exit_max):
                    continue

                # -----------------------------
                # Shock-count constraint
                # -----------------------------
                ...
                num_oblique = sum(
                    1
                    for s in r["stages"]
                    if "Normal Shock" not in s["stage"]
                )
...
if __name__ == "__main__":
    best = optimise_intake_fixed_L1(
        M_inf=2.5,
        L1=0.3,
        delta_cowl_deg=0,
        max_oblique_shocks=5,
        ycowl_range=(0.05, 2),
        theta1_range=(2, 15),
        ycowl_step=0.01,
        theta1_step=0.25,
    ...
    analyse_intake4(M_inf=2.5, L_1 = 0.3, theta_1_deg = 7.5, y_cowl = 0.41, verbose = True)