...
class Ramjet:
    ...
    # Design point used to size the inlet capture area.  Existing calls that
    ...
    def inlet_properties(
        ...
        *,
        ...
        if use_fixed_capture_area:
            # Preserve old call style: inlet_properties(..., m_air=100)
            # now means "size the fixed capture area for 100 kg/s at the
...
if __name__ == "__main__":
    ...
    print(f"  Flight: h={h_km} km, Ma₀={Ma0}, ramjet design ṁ_air={mdot} kg/s")
    ...
    print(f"\n  Sec4 inlet of_ratio = {(inp['mdot'])/(sec3['mfuel']+1e-30):.2f}")
