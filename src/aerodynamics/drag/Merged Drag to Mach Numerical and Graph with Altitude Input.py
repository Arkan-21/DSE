# -*- coding: utf-8 -*-
"""
Created on Tue May 19 14:11:42 2026
...
fixed_altitude_m = 42000.0  # Set to any altitude in meters (e.g., 25000.0)
...
# Pre-calculate uniform atmospheric state for the run
...
for idx, M in enumerate(mach_range):
    ...
    # --- REGIME 2 & 3: SMOOTHLY BLENDED SUPERSONIC TO HYPERSONIC (M >= 1.2) ---
        ...
        # Wrapped in an absolute value block to safeguard against negative square roots
    ...
    # --- Live Continuous Numerical Stream Output ---
    if idx % 4 == 0 or M == 6.0:
...
# Convert results dictionary to numpy arrays to enable robust plotting filtering
for key in results:
    if key != 'regime':
...
# =============================================================================
# 5. CONTINUOUS FLIGHT ENVELOPE VISUALIZATION (UNCAPPED GRAPH FILTER)
# =============================================================================
# Filters out unphysical asymptotic math spikes solely for clean display scaling
valid_mask = results['cd'] < 50.0  
alpha_mask = results['alpha'] < 90.0
thrust_mask = results['thrust'] < 1e5
