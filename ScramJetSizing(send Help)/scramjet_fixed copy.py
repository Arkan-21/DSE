...
class ShapiroODE:
    ...
    def integrate(x_start, x_end,
        ...
        # Just above sonic to avoid the M=1 singularity in Shapiro's denominator
        y0 = [
            max(Ma2_in, 1.00001),
            max(p_in, 1.0),
            max(T_in, 1.0),
        ...
        sol = solve_ivp(
            ...
            events=[choke_event, temperature_event, pressure_event],
            dense_output=False,
        )
        ...
        return {
            ...
            "Ma": Mas,
            "Ma2": Ma2s,
            # Provide both lowercase and uppercase pressure keys so downstream
            # consumers (plot_flowpath etc.) can use either convention.
            "p": ps,
            "P": ps,
            "T": Ts,
            "ht": Ht0,
...
class Engine:
    ...
    def inlet_properties(self, h, Ma, m_air):
        ...
        # Both unsubscripted and subscripted (...0) names are returned so that
        # the rest of the pipeline can use whichever convention.
        return {
            "Ma": Ma,    "Ma0": Ma,
            "T":  T0,    "T0":  T0,
            "P":  P0,    "P0":  P0,
    ...
    def isolator_properties(self, inlet_props):
        ...
        # Linear two-point "solution" used only for plotting.
        ...
        return {
            # Unsubscripted (used internally by downstream stages)…
            "Ma": M1,
            "T":  T1,
            "Tt": Tt1,
            "P":  P1,
            ...
            "sigma_c": sigma_c,
            "mdot": mdot,
            # …plus subscripted ("...1") aliases used by the report printout.
            "Ma1": M1,
            "T1":  T1,
            "p1":  P1,
            "P1":  P1,
    ...
    def combustor_properties2(self, isolator_props):
        ...
        return {
            ...
            "A": A2,
            "gamma": self.air.specific_heat_ratio(T_end, p_end),
            "cp": self.air.specific_cp(T_end, p_end),
            "R": self.air.specific_R(T_end, p_end),
            "mdot": mdot,
            "solution": result,
        }
    ...
    def combustor_properties3(self, combustor_properties2, phi=0.1):
        """Section 2→3: Fuel injection (mass addition only, evolving mixture properties)."""
        ...
        result = self.shapiroODE.integrate(
            x_start=0.0, x_end=L_23, Ma2_in=Ma2**2, p_in=p2, T_in=T2, mdot_in=mdot_air,
            geometry_fn=geometry_fn, thermo_fn=thermo_fn, source_fn=source_fn,
    ...
    def combustor_properties4(self, combustor_properties3):
        ...
        Q_H2 = 120e6  # J/kg (HHV, paper Table 3)
        ...
        def mixing_efficiency(x):
            ...
            a = np.clip(1.01 + 0.176 * np.log(s), 0.0, 1.0)
            print(f"x={x:.4f} m, s={s:.4f}")
            if theta == 0.0:
                return s
            elif theta == 90.0:
                a 
    ...
    def performance(self, inlet_props, nozzle_props, combustor3_props):
        """Compute Fin and Ia per paper eqs.(33)–(37)"""
    ...
    def plot_flowpath(self, inp, iso, sec2, sec3, sec4, sec5=None):
        ...
        def add_section(sol, x_offset):
            # ShapiroODE results expose both `p`/`P` and `pt`/`Pt`. The
            # isolator's hand-rolled solution dict also exposes both. Falling
            # back through the alternatives keeps this robust.
        ...
        mdot = np.concatenate([s["mdot"] for s in sections])
                ...
                # -------------------------------------------------
        # Engine geometry reconstruction
        # -------------------------------------------------

        A_geom = []

        # Isolator
        A_geom.extend(iso["solution"]["A"])

        # Section 2
        A_geom.extend(sec2["solution"]["A"])

        # Section 3
        A_geom.extend(sec3["solution"]["A"])

        # Section 4
        A_geom.extend(sec4["solution"]["A"])

        # Nozzle
        if sec5 is not None and not sec4.get("thermal_choke", False):
            A_geom.extend(sec5["solution"]["A"])

        A_geom = np.array(A_geom)

        # Equivalent radius from area
        r_geom = np.sqrt(A_geom / np.pi)

        # Normalize for plotting
        r_geom = r_geom / np.max(r_geom)
        ...
        axs[3].plot(x, V, lw=2, color='tab:blue')
        axs[3].set_ylabel("Velocity [m/s]")

        axs[4].plot(x, mdot, lw=2, color='tab:purple')
        ...
        geom_scale = 0.45 * np.max(mdot)

        upper = geom_scale * r_geom
        lower = -geom_scale * r_geom
        ...
        axs[4].fill_between(
            ...
            color='lightgray',
            alpha=0.35,
            label='Engine Geometry'
        )

        axs[4].plot(x, upper, color='black', lw=1.5)
        axs[4].plot(x, lower, color='black', lw=1.5)
...
if __name__ == "__main__":
    ...
    if sec4["thermal_choke"]:
        ...
        print_section("Section 2 — Isolator exit", sec2, [
            ("Mach number Ma₂", "Ma2", "—", 1.0),
            ("Temperature T₂", "T2", "K", 1.0),
            ("Pressure p₂", "p2", "kPa", 1e-3),
            ("Velocity V₂", "V2", "m/s", 1.0),
        ])
        print_section("Section 3 — Combustor fuel injection", sec3, [
