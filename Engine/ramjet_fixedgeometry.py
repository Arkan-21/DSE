...
class MixtureNASA:
    ...
    def stagnation_Tt(self, Y, T_static, h_target, tol=1e-3, max_iter=60):
        ...
        for _ in range(max_iter):
            ...
            delta = max(-400.0, min(800.0, delta))
...
class Ramjet:
    """
    1-D Ramjet cycle model — fixed geometry.

    The inlet capture area A0 is a fixed geometric parameter.
    Mass flow is derived from A0 and the freestream conditions:
        ṁ = ρ₀ · V₀ · A0
    """
    ...
    # ── Fixed inlet capture area [m²] ─────────────────────────────────────
    A0  = 8.0   # <-- SET THIS to your physical inlet area
    ...
    # =====================================================================
    def inlet_properties(self, h, Ma):
        """
        Compute inlet state from fixed geometry A0 and flight conditions.
        Mass flow is derived as  ṁ = ρ₀ · V₀ · A0  (not an input).
        """
        T0   = Atmosphere.T(h)
        P0   = Atmosphere.P(h)
        ...
        V0  = Ma * a0
        A0  = float(self.A0)           # fixed geometry
        m_air = rho0 * V0 * A0        # derived from area, not prescribed
        ...
        print(f"\n── Inlet  h={h:.0f} m  Ma={Ma:.2f}  A0={A0:.4f} m² ──")
        print(f"  T0={T0:.1f} K   P0={P0:.0f} Pa   rho0={rho0:.4f} kg/m³")
        print(f"  V0={V0:.1f} m/s   ṁ={m_air:.2f} kg/s   Tt0={Tt0:.1f} K   Pt0={Pt0:.0f} Pa")
    ...
    # =====================================================================
    # Section 3→4 — Combustion (CEA equilibrium)
    # =====================================================================
    ...
    # =====================================================================
    # Section 4→5 — Convergent-divergent nozzle
    # =====================================================================
    def nozzle_properties(self, sec4, inlet_props, switches=None):
        ...
        A5   = self._f(self.alpha05) * A0   # fixed exit area from geometry
    ...
    def performance(self, inlet_props, nozzle_props, sec3):
        ...
        Ia  = Fin / mdot_air
        return {
            "Fin": Fin, "Isp": Isp, "Ia": Ia,
            "mfuel": mfuel, "mdot_air": mdot_air,
            "A0": A0, "A5": A5,
            "thermal_choke": False,
        }
    ...
    def altitude_mach(self, h_km, Ma0):
        """Run a single case at given altitude and Mach. Mass flow from A0."""
        inp  = self.inlet_properties(h=h_km*1e3, Ma=Ma0)
        iso  = self.isolator_properties(inp)
        sec2 = self.combustor_properties2(iso)
        sec3 = self.combustor_properties3(sec2, phi=0.5)
...
if __name__ == "__main__":
    ...
    # ── Fixed geometry ─────────────────────────────────────────────────────
    eng.A0  = 8   # inlet capture area [m²]  ← set your geometry here
    ...
    print(f"\n{'═'*65}")
    print(f"  RAMJET PERFORMANCE ANALYSIS (H₂ fuel, φ={phi})")
    print(f"  Flight: h={h_km} km, Ma₀={Ma0},  A0={eng.A0} m²")
    print(f"  CEA-backed equilibrium combustion + integral-form Tt/Pt")
    print(f"{'═'*65}")

    inp  = eng.inlet_properties(h=h_km*1e3, Ma=Ma0)   # no m_air argument
    ...
    print_section("Section 0 — Freestream", inp, [
        ...
        ("Inlet area A₀",          "A0",   "m²",     1.0),
        ("Mass flow ṁ",            "mdot", "kg/s",   1.0),
    ...
    print_section("Section 3 — Fuel injection exit", sec3, [
    ...
    print_section("PERFORMANCE METRICS", perf, [
        ...
        ("Air mass flow ṁ_air",    "mdot_air", "kg/s",   1.0),
        ("Inlet area A₀",          "A0",       "m²",     1.0),
        ("Exit area A₅",           "A5",       "m²",     1.0),
    ])

    print(f"\n  Fuel: H₂,  φ={phi},  FAR={sec3['mfuel']/inp['mdot']:.5f}")
    print(f"  ṁ_fuel = {sec3['mfuel']:.4f} kg/s")
    print(f"  ṁ_air  = {inp['mdot']:.4f} kg/s")
...
# ---------------------------------------------------------------------------
def plot_vs_mach(eng, mach_range=None, h_km=25.0, phi=0.5):
    """Thrust and Isp vs Mach number at fixed altitude."""
    if mach_range is None:
        mach_range = np.arange(2.0, 5.5, 0.25)

    Fins, Isps = [], []
    for Ma in mach_range:
        try:
            inp  = eng.inlet_properties(h=h_km*1e3, Ma=Ma)
            iso  = eng.isolator_properties(inp)
            sec2 = eng.combustor_properties2(iso)
            sec3 = eng.combustor_properties3(sec2, phi=phi)
            sec4 = eng.combustor_properties4(sec3)
            sec5 = eng.nozzle_properties(sec4, inp)
            perf = eng.performance(inp, sec5, sec3)
            Fins.append(perf["Fin"])
            Isps.append(perf["Isp"])
        except Exception as e:
            print(f"  Skipping Ma={Ma:.1f}: {e}")
            Fins.append(np.nan)
            Isps.append(np.nan)
    ...
    ax1.plot(mach_range, Fins, color="black", lw=2,       label="Thrust (N)")
    ax2.plot(mach_range, Isps, color="black", lw=2, ls="--", label="Isp (s)")

    ax1.set_xlabel("Mach Number  $Ma_0$")
    ax1.set_ylabel("Thrust  [N]")
    ax2.set_ylabel("Specific Impulse  [s]")
    ax1.set_title(f"Ramjet Performance vs Mach  (h = {h_km} km,  φ = {phi},  A₀ = {eng.A0} m²)")
...
def plot_vs_altitude(eng, alt_range=None, Ma0=3.0, phi=0.5):
    """Thrust and Isp vs altitude at fixed Mach number."""
    if alt_range is None:
        alt_range = np.arange(10.0, 32.0, 1.0)

    Fins, Isps = [], []
    for h_km in alt_range:
        try:
            inp  = eng.inlet_properties(h=h_km*1e3, Ma=Ma0)
            iso  = eng.isolator_properties(inp)
            sec2 = eng.combustor_properties2(iso)
            sec3 = eng.combustor_properties3(sec2, phi=phi)
            sec4 = eng.combustor_properties4(sec3)
            ...
            print(f"  Skipping h={h_km:.0f} km: {e}")
            Fins.append(np.nan)
            Isps.append(np.nan)
    ...
    ax1.plot(alt_range, Fins, color="black", lw=2,       label="Thrust (N)")
    ax2.plot(alt_range, Isps, color="black", lw=2, ls="--", label="Isp (s)")

    ax1.set_xlabel("Altitude  [km]")
    ax1.set_ylabel("Thrust  [N]")
    ax2.set_ylabel("Specific Impulse  [s]")
    ax1.set_title(f"Ramjet Performance vs Altitude  (Ma₀ = {Ma0},  φ = {phi},  A₀ = {eng.A0} m²)")
...
if __name__ == "__main__":
    ...
    plot_vs_mach(eng, mach_range=np.arange(2.0, 5.5, 0.5), h_km=25.0, phi=phi)
    plot_vs_altitude(eng, alt_range=np.arange(10.0, 32.0, 2.0), Ma0=3.0, phi=phi)